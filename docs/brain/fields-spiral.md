...
- **Ritorno e Overscroll** (dalle pagine di dettaglio):
  - **Overscroll in alto (Pull-to-return)**: superando il bordo superiore della pagina su desktop (via wheel/trackpad), al raggiungimento della soglia viene attivata fluidamente la transizione verso `/fields`. Lo stato dell'ultimo articolo visualizzato viene salvato in `sessionStorage` (`fields-target-marker`) e la spirale 3D in `/fields` si riapre ripristinando l'angolo, il progresso e lo scroll esatto (`ScrollTrigger`) del marker corrispondente, senza dover rigenerare l'animazione di intro da capo.
  - **Overscroll in basso (Pull-to-next)**: superando il fondo della pagina oltre il contenuto su desktop (via wheel/trackpad), al superamento della soglia anima il contenuto verso l'alto con slide verticale fluido e naviga all'articolo successivo secondo l'ordine cronologico globale della timeline di Fields.
  - **Indicatore di carica (deciso il 2026-08-21, sostituisce la "pura serendipità" senza feedback)**: entrambi gli overscroll mostrano una freccia guidata dallo scroll — `↑` in alto, `↓` in basso — in `position: fixed`, centrata orizzontalmente, a `top-20 sm:top-28` / `bottom-20 sm:bottom-28` (in alto deve scavalcare la navbar, che è alta ~56px sotto `sm` e ~80px sopra; in basso la stessa quota per simmetria). Glifo di testo 28px, stesso linguaggio dei link "← Indietro" / "Successivo ↓" a fondo pagina. **Varia solo l'opacità** (`0.15 + 0.85 × progresso`): niente movimento, niente scala, niente ring SVG. Nessuna transizione mentre carica (deve seguire il gesto), `.3s` al rilascio. La freccia in basso non esiste sull'ultimo articolo del field: dove non c'è azione non c'è indicatore.
  - **Taratura del gesto**: soglia `2000` di `deltaY` grezzo accumulato (era `90` su `deltaY * 0.4`, ~9x più sensibile: bastava un colpo di trackpad per cambiare pagina senza accorgersene). L'accumulo si azzera appena il gesto smette di spingere contro il bordo e comunque dopo 400ms di quiete — prima sopravviveva indefinitamente e sommava gesti scollegati. L'accumulo parte solo dopo 200ms dall'ultimo scroll "libero", così l'inerzia del trackpad che arriva a fondo pagina non carica nulla: bisogna ricominciare a spingere. Al raggiungimento della soglia l'azione parte subito, non c'è stato "armato": su `wheel` non esiste un rilascio da intercettare.
  - **Mobile e Touch**: i listener touch per l'overscroll sono disattivati per evitare qualsiasi interferenza con i gesti e controlli nativi dei browser mobile (pull-to-refresh, barra degli indirizzi, swipe di navigazione). Su smartphone e tablet la navigazione tra articoli e il ritorno a Fields avvengono tramite i pulsanti dedicati a fondo pagina ("← Indietro" e "Successivo ↓").
